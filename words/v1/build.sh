...
docker stop ${id}
docker rm ${id}
docker rmi acmegrocery/smartcart:v1
echo *INFO* building a new NodeJS container image
docker build -t acmegrocery/smartcart:v1 .
sudo docker push acmegrocery/smartcart:v1

exit 0

echo *INFO* running our new image and exposing service on port 2020
docker run -p 2020:8080 -d acmegrocery/smartcart:v1
